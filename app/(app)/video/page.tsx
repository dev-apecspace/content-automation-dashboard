...
export default function VideoPage() {
  ...
  const handleApproveVideo = async (item: VideoItem) => {
    try {
      ...
      setVideoItems((prev) => prev.map((v) => (v.id === item.id ? updated : v)))
  ...
  const handleSaveVideo = async (data: Partial<VideoItem>) => {
    try {
      ...
      if (editVideo) {
        ...
        setVideoItems((prev) => prev.map((v) => (v.id === editVideo.id ? updated : v)))
        ...
        const newVideo = await createVideoItem({
          ...
          platform: (data.platform as "Facebook Reels" | "Youtube Shorts") || "Facebook Reels",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Quản lý video (Reels, Shorts) và phê duyệt</p>
        </div>
